...
export const apiClient =  axios.create({
  baseURL: "http://192.168.116.44:3001/api/v1",
  headers: {
 ...
 */
 const checkingValidation = async () => {
  ...
  try {
    const result = await apiClient.post("/auth/isValidToken", {token : _token})
    if (result.data.ok) {
...
export const isUserLoggedIn = () => {
  checkingValidation()
  return JSON.parse(localStorage.getItem("validToken"))
}
