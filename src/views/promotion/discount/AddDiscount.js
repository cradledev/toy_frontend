...
const AddNewProductModal = (props) => {
    ...
    useEffect(() => {
        if (!store.addStatus) {
            setDescription("")
            setTitle("")
            setDiscountType("AssignedToProducts")
            setUsePercentage(false)
            setDiscountValue(0)
            setCouponCode("")
            setUseCouponCode(false)
            setStartTime("")
            setEndTime("")
            setDiscountLimitation("NtimesPerCustomer")
            setTimes(0)
            setMinOrderSubTotalAmount(0)
            setMinOrderTotalAmount(0)
            setStatus(true)
        }
    }, [store.addStatus])
    ...
    const onSubmit = async (event) =>  {
        ...
        dispatch(saveDiscount({...putData}))
        dispatch(addStarting(false))
    }

    const getCouponCode = async () => {
        const result = await apiClient.get("/discount/couponCode")
        const _couponCode = result.data.couponCode
        setCouponCode(_couponCode.toString().toUpperCase())
    }
    return (
        <Fragment>
            <Modal isOpen={open} toggle={toggleModal} className='modal-dialog-centered modal-lg'>
                ...
                <div className='text-center mb-2'>
                    <h1 className='mb-1'>Add New Discount Detail</h1>
                    <p>Adding new Discount details</p>
                </div>
